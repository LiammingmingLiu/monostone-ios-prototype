...
> 你把转写文本压成 description — 单一主题的可检索记忆原子。
>
> **输出字段**
> - `title` ≤ 12 字人话标题
> - `text` 两段（\n\n 分隔）：≤ 30 字摘要 + 详细内容（保留所有人名/数字/决策/时间）
> - `importance` 0-1，Agent 未来检索这条的价值（不是用户主观重要度）
> - `entities` 人/项目/地点/时间/数字
>
> **拆条**
> 只在主题真切换时拆。会议中多人交替、对话往返、停顿都不是拆点。强信号：议程过渡词（"对了"、"换个话题"）、新议题独立提起、跨场景或跨日。难判定就不拆。
>
> **取舍**
> 丢寒暄、口头禅、重复。保留判断和情绪信号。

## 决策规则
- 一段 raw 必拆出 ≥ 1 条 description
- 拆 = 主题切换（仅此唯一硬规则）；时间/角色切换不独立触发
- title 和 text 第一段是同一意思的两种长度表达
- 用户编辑 title/text 后系统不重新压缩
...
### Example 6：⚠️ 同主题多人会议（不拆，验证拆点规则）

**Input** (10 分钟会议，3 人交替发言，全程同主题):
```
（00:00）我：续航这周测出来 38 小时。
（00:10）林啸：嗯，比上版好但还差 12 小时。
（00:30）我：BLE 那块呢？
（00:42）林啸：我准备把心率上报频率从 1Hz 降到 0.2Hz。
（01:00）Sean：这样会影响 UI 显示吗？心率图表可能就不连续了。
（01:15）林啸：会的。但用户感受不到 — 我们端上做插值。
（01:30）我：插值会不会让数据不准？
（01:45）林啸：误差在 2% 以内，我对比过。
（02:00）Sean：那 UI 这边我要改吗？
（02:15）林啸：不用，纯后端的事。
（03:00）林啸：还有 PHY 时钟也能再降 5%。
（03:30）我：明天测试一起看看。
（08:00）我：还能再省 5% 就 50 小时达标了。
```
...
```json
{
  "description_nodes": [
    {
      "title": "续航优化讨论",
      "text": "续航 38h vs 50h 目标，BLE 降频 + PHY 时钟方案确认\n\n10 分钟会议讨论续航优化：当前 38h，目标 50h，差 12h。林啸方案：心率上报 1Hz→0.2Hz（端侧插值，误差 2% 内，UI 无需改）+ PHY 时钟再降 5%。明天测试验证。多人交替发言但全程同主题。",
      "importance": 0.85,
      "entities": ["林啸", "Sean", "续航", "BLE", "心率上报", "PHY 时钟"]
    }
  ],
  "schema_version": 1
}
...
**为什么不拆**：
- 你 / 林啸 / Sean 三人交替发言，**角色切换多次但话题没变**（都是续航优化）
- 8 分钟时间间隔 → 弱信号（同主题继续，不拆）
- 没有强信号（没有"对了 / 另外"等议程过渡词、没有新主题独立提起）
- → 1 条 description 即可
- importance 0.85（高信息密度 + 决策方向）

⚠️ **反例**：如果同样的对话最后加上 "对了，Sean 推了个 iOS 候选人，下周想聊一下" → 这是"议程过渡词 + 新主题"强信号 → 拆成 2 条（参考 Example 1）。
