...
| 来源子类 | 标题（≤ 10 字，简短说明是什么事） | 副信息 | chip |
|---|---|---|---|
| 长录音卡 | `{长录音简短主题}` | `{N} 分钟录音` | 卡片 |
| 指令卡 | `{指令简短主题}` | `指令` | 卡片 |
| 灵感卡 | `{灵感简短主题}` | `灵感` | 卡片 |
| 待办卡 | `{待办简短主题}` | `待办` | 卡片 |
| **Preference** | `{Agent 自动生成的简短描述，不写"偏好"两字}` | `Memory` | Memory |
| **人际关系** | `{Agent 自动生成的简短描述，不写人名}` | `Memory` | Memory |
| **MCP 外部数据** | `{Agent 自动生成的简短描述}` | `Memory` | Memory |
